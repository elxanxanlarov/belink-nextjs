...
import { privacyModalData } from "@/data/modalData";
import { BaseModalProps } from "@/types";

export default function PrivacyModal({ isOpen, onClose }: BaseModalProps) {
    ...
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-lg max-h-[85vh] flex flex-col animate-in fade-in zoom-in-95 duration-200">
        <div className="flex items-center justify-between px-7 py-5 border-b border-gray-100 flex-shrink-0">
          <h2 className="text-lg font-extrabold text-gray-900">{privacyModalData.title}</h2>
          <button
            onClick={onClose}
        ...
        <div className="overflow-y-auto px-7 py-6 flex flex-col gap-5">
          <p className="text-xs text-gray-400 font-medium">{privacyModalData.lastUpdated}</p>
          {privacyModalData.sections.map((section, i) => (
